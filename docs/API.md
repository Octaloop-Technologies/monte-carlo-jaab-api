...
- `portfolio` — `PortfolioMetrics`: joint breach probabilities, **`min_dscr_across_assets`** vs **`max_dscr_across_assets`** vs **`revenue_weighted_mean_dscr_across_assets`** (same revenue weights as HHI / weighted breach—per-scenario weakest, strongest, and blended DSCR paths, then summarized), **`revenue_weighted_mean_equity_irr_across_assets`** plus **`var_irr_95`** / **`cvar_irr_95`** on that blend (same definitions as per-asset IRR tail metrics—not consolidated-fund IRR), concentration / weighted breach, summed CF tails, plus **cross-asset Pearson matrices** `cross_asset_dscr_correlation_pearson` / `cross_asset_equity_irr_correlation_pearson` (order = `metadata.asset_ids`) — **not** the 4×4 factor `correlation` in the shock pack.
